...
// Retry policy: for recognition we retry on connection failures only.
// audd-go separates classes (mutating / read / recognition) but the
// difference rarely matters in user-facing behavior. We unify here: retry
// on AuddConnectionError up to max_attempts, with linear backoff.
template <typename F>
auto retry_do(int max_attempts, std::chrono::milliseconds backoff, F&& f)
    -> decltype(f()) {
    int attempt = 0;
...
nlohmann::json AudD::Internal::post_form(const std::string& url,
    ...
    auto resp = retry_do(config.max_attempts, config.backoff_factor,
...
AudD::recognize(const Source& source, const RecognizeOptions& opts) {
    ...
    try {
        resp = retry_do(internal_->config.max_attempts, internal_->config.backoff_factor,
            [&]() -> internal::HttpResponse {
                auto fields = reopen();
