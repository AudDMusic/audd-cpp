...
TEST_CASE("version constant is exposed and shaped like a semver") {
    const std::string v(audd::version());
    CHECK_FALSE(v.empty());
    CHECK(v == AUDD_VERSION);
    // Semver MAJOR.MINOR.PATCH prefix:
    const auto first_dot = v.find('.');
    const auto second_dot = v.find('.', first_dot + 1);
    CHECK(first_dot != std::string::npos);
    CHECK(second_dot != std::string::npos);
}
